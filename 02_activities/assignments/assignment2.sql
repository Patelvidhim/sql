...
VALUES (
    15,              -- Replace with an appropriate product_id (e.g., 1001)
    'Apple Pie',       -- Product name
    'Medium',          -- Product size
    'unit',            -- Product quantity type
    CURRENT_TIMESTAMP  -- Timestamp of when the row is inserted
);
...
WHERE product_id = 15
AND snapshot_timestamp < (SELECT MAX(snapshot_timestamp) FROM product_units WHERE product_id = 15);
